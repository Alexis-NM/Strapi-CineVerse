...
export default function MovieOverlay({ movie, onClose }) {
  ...
  const duration = movie.duration_minutes ? `${Math.floor(movie.duration_minutes/60)}h${(movie.duration_minutes%60).toString().padStart(2,'0')}` : 'N/A';
    ...
    >
      <div className="bg-[#0b0b0b] rounded-2xl overflow-hidden shadow-2xl mt-6 max-w-[1000px] xl:max-w-[1080px] mx-auto">
        <div className="relative w-full h-80 md:h-[420px] bg-black">
          {playingTrailer ? (
            (() => {
              const src = getYouTubeEmbedUrl(movie.trailer_url || movie.trailer || movie.youtubeId || movie.youtube_id);
              return src ? (
                <iframe
                  title={`trailer-${title}`}
                  src={src}
                ...
                <div className="w-full h-full bg-gray-800 flex items-center justify-center text-gray-400">No trailer available</div>
              );
            })()
          ) : poster ? (
          ...
          {/* boutons centrés en bas de l'image */}
            <div className="absolute left-1/2 bottom-6 transform -translate-x-1/2 flex items-center gap-3">
              <button
                ...
                onClick={() => {
                  const src = getYouTubeEmbedUrl(movie.trailer_url || movie.trailer || movie.youtubeId || movie.youtube_id);
                  console.log('Watch Trailer clicked, embed src=', src, 'movie=', movie);
